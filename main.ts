...
function outofscreendeath (end: Sprite) {
    for (let index = 0; index <= Playa_List.length; index++) {
        xdist = camera.x - end.y
        ydist = camera.x - end.y
        Distance = Math.sqrt(xdist * 2 + ydist * 2)
        if (Distance < 40) {
        	
        } else {
            sprites.destroy(Playa_List[index])
        }
    }
}
controller.menu.onEvent(ControllerButtonEvent.Pressed, function () {
    ...
    scene.setBackgroundImage(img`
        ...
        `)
    sprites.destroy(SpriteText, effects.fire, 500)
    camera = sprites.create(img`
        ...
        . . . . . . . . . . . . . . . . 
        `, SpriteKind.Camera)
    camera.setVelocity(0, -30)
    tiles.placeOnTile(camera, tiles.getTileLocation(5, 253))
    ...
    mp.setPlayerSprite(mp.playerSelector(mp.PlayerNumber.One), sprites.create(img`
        ...
        . . . . . f f . . f f . . . . . 
        `, SpriteKind.Player))
    tiles.placeOnTile(mp.getPlayerSprite(mp.playerSelector(mp.PlayerNumber.One)), tiles.getTileLocation(randint(0, 9), 253))
    mp.setPlayerSprite(mp.playerSelector(mp.PlayerNumber.Two), sprites.create(img`
        ...
        . . . . . f f . . f f . . . . . 
        `, SpriteKind.Player))
    tiles.placeOnTile(mp.getPlayerSprite(mp.playerSelector(mp.PlayerNumber.Two)), tiles.getTileLocation(0, 253))
    mp.setPlayerSprite(mp.playerSelector(mp.PlayerNumber.Three), sprites.create(img`
        ...
        . . . . . f f b b f f . . . . . 
        `, SpriteKind.Player))
    tiles.placeOnTile(mp.getPlayerSprite(mp.playerSelector(mp.PlayerNumber.Three)), tiles.getTileLocation(randint(0, 9), 253))
    mp.setPlayerSprite(mp.playerSelector(mp.PlayerNumber.Four), sprites.create(img`
        ...
        `, SpriteKind.Player))
    tiles.placeOnTile(mp.getPlayerSprite(mp.playerSelector(mp.PlayerNumber.Four)), tiles.getTileLocation(randint(0, 9), 253))
    Playa_List = [
    ...
    mp.getPlayerSprite(mp.playerSelector(mp.PlayerNumber.Three)).setStayInScreen(false)
    mp.getPlayerSprite(mp.playerSelector(mp.PlayerNumber.Four)).setStayInScreen(false)
    GeneratedTilemap(1)
    mp.getPlayerSprite(mp.playerSelector(mp.PlayerNumber.One)).ay = 500
    mp.getPlayerSprite(mp.playerSelector(mp.PlayerNumber.Two)).ay = 270
    mp.getPlayerSprite(mp.playerSelector(mp.PlayerNumber.Three)).ay = 270
    mp.getPlayerSprite(mp.playerSelector(mp.PlayerNumber.Four)).ay = 270
...
function GeneratedTilemap (position: number) {
    currentposition = tiles.locationOfSprite(camera)
    while (tiles.locationXY(currentposition, tiles.XY.column) > 0) {
        for (let index = 0; index < 3; index++) {
            currentposition = tiles.locationInDirection(currentposition, CollisionDirection.Top)
        }
        if (Math.percentChance(randint(0, 50))) {
            movingdirection = CollisionDirection.Left
        } else {
            movingdirection = CollisionDirection.Right
        }
        for (let index = 0; index < randint(3, 12); index++) {
            if (tiles.locationXY(currentposition, tiles.XY.column) > 0 && tiles.locationXY(currentposition, tiles.XY.column) < tiles.tilemapColumns() - 4) {
                currentposition = tiles.locationInDirection(currentposition, movingdirection)
            } else if (tiles.locationXY(currentposition, tiles.XY.column) == 0) {
                currentposition = tiles.locationInDirection(currentposition, CollisionDirection.Bottom)
            } else {
                currentposition = tiles.locationInDirection(currentposition, CollisionDirection.Left)
            }
        }
        for (let index = 0; index < 3; index++) {
            tiles.setTileAt(currentposition, sprites.dungeon.stairEast)
            tiles.setWallAt(currentposition, true)
            currentposition = tiles.locationInDirection(currentposition, CollisionDirection.Left)
        }
        for (let index = 0; index < randint(1, 4); index++) {
            if (Math.percentChance(randint(25, 50))) {
                currentposition = tiles.locationInDirection(currentposition, CollisionDirection.Left)
            } else {
...
SpriteText.sayText("フォール・シュミレーターへようこそ", 2000, false)
forever(function () {
    if (controller.menu.isPressed()) {
        outofscreendeath(camera)
    }
})
